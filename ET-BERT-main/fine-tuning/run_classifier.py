...
def evaluate(args, dataset, print_confusion_matrix=False):
    ...
    if print_confusion_matrix:
        ...
        confusion_dir = os.path.dirname(args.output_model_path) or "."
        os.makedirs(confusion_dir, exist_ok=True)
        confusion_path = os.path.join(confusion_dir, "confusion_matrix.txt")
        with open(confusion_path, 'w', encoding='utf-8') as f:
            for cf_a in cf_array:
                f.write(str(cf_a) + '\n')
        print("Confusion matrix saved to {}".format(confusion_path))
