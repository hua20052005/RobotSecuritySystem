...
const threatChart = useChart(threatRef)
const protocolChart = useChart(protocolRef)
...
const renderCharts = () => {
  ...
  threatChart.setOption({
    ...
    xAxis: { type: 'category', data: Object.keys(threatDist), axisLabel: { color: '#6b645d' } },
    yAxis: { axisLabel: { color: '#6b645d' } },
    series: [
      {
        type: 'bar',
        data: Object.values(threatDist),
  ...
  protocolChart.setOption({
    ...
    yAxis: {
      type: 'category',
      data: Object.keys(protocolDist),
      axisLabel: { color: '#6b645d' },
    },
    series: [
      {
        type: 'bar',
        data: Object.values(protocolDist),
...
const runDetection = async () => {
  ...
  try {
    ...
    ElMessage.error(errorText(error, '载荷检测失败，请检查后端日志。'))
  } finally {
    loading.value = false
  }
}
...
const generateReport = async () => {
  ...
  try {
    ...
    ElMessage.error(errorText(error, 'AI 报告生成失败，请检查后端配置。'))
  } finally {
...
<template>
  <section
    ...
    element-loading-text="正在逐包检测载荷，文件较大时可能需要 1-2 分钟…"
  ...
  >
    ...
    <div class="grid-2 mt-20">
      ...
      <div>
        ...
        <p class="panel-sub mt-18">详细日志</p>
  ...
  <section v-if="result" class="panel fade-in">
    ...
    <div class="data-table mt-16">
      <el-table :data="result.preview" max-height="360" stripe>
        <el-table-column
