...
const publishStatus = (data) => {
  status.value = data
  emit('status', data)
  if (ownsSession.value && data.latest_result && data.latest_result.run_id !== lastResultId) {
...
const start = async () => {
  ...
  try {
    ...
    publishStatus(data)
    ElMessage.success('实验设备已接入')
  } catch (error) {
    ElMessage.error(errorText(error, '实验设备接入失败'))
    await poll()
  } finally {
    busy.value = false
  }
...
const stop = async () => {
  ...
  try {
    ...
    publishStatus(data)
    ElMessage.success('接入已断开')
  } catch (error) {
    ElMessage.error(errorText(error, '断开失败'))
  } finally {
    busy.value = false
  }
}
  ...
  <div class="live-capture-panel">
    <div class="live-settings">
      <div class="live-heading">
        <div>
          <strong>实验设备接入</strong>
          <span>由后端建立受控连接并采集当前链路窗口</span>
        </div>
    ...
    <div class="live-status">
      ...
      <div class="status-copy">
        <strong>{{ occupied ? `${moduleLabel[status?.config?.module] || '其他模块'}正在使用接入通道` : status?.message || '尚未接入' }}</strong>
